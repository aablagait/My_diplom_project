...
# функция по детектированию объектов
...
# функция для удаления объектов не переступивших порог
...
def box_drawing(input_frame, indices, bounding_boxes, class_objects, confidence_probs, classNames, classification_list, color=(0, 255, 255),
                thickness=2):
    ...
    for i in indices:
        ...
        cv2.putText(input_frame, f'{classNames[class_objects[i]].upper()} {int(confidence_probs[i] * 100)}% {classification_list[a][0]} {classification_list[a][1]}%',
                    (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, thickness=1)
        a+=1


# конвертация входного изображения в blob формат для OpenCV DNN
